...
section .data
print_int_buffer    dq    0     ; 定义8字节缓冲区用于数字到字符的转换

[bits 32]
section .text
;--------------------------------------------
;print_str 通过print_char来打印以0字符结尾的字符串
;--------------------------------------------
;输入：栈中参数为打印的字符串
;输出：无
...
cls_screen:
   ...
   ;;;;;;;;;;;;;;;
	...
	; 导致用户程序再次进入中断后,gs为0,故直接在print_str中每次都为gs赋值. 
...
print_int:
   ...
   mov edx, eax
   mov edi, 7                          ; 指定在print_int_buffer中初始的偏移量
...
;将每一位数字转换成对应的字符后,按照类似“大端”的顺序存储到缓冲区print_int_buffer
...
.print_each_num:
   ...
   add esp, 4
   inc edi			       ; 使edi指向下一个字符
   mov cl, [print_int_buffer+edi]	       ; 获取下一个字符到cl寄存器
   cmp edi,8
   jl .print_each_num
   popad
   ret
