...
struct partition* cur_part;	 /*一般情况下操作的分区*/
...
static void partition_format(struct partition* part) 
{
    ...
    /*inode bitmap占用一个扇区，意味着只支持4096个文件*/
...
/*********************************************************/
    ...
    sb.inode_cnt = MAX_FILES_PER_PART;  /*最多4096个*/
    sb.part_lba_base = part->start_lba; /*起始扇区编号*/
    ...
    sb.root_inode_no = 0;   /*根目录的number = 0*/
    sb.dir_entry_size = sizeof(struct dir_entry);/*在超级块中记录目录项大小*/
...
int32_t sys_write(int32_t fd, const void* buf, uint32_t count) 
{
    ...
    { 
        ...
        {
            console_print_str((char*)"sys_write: not allowed to write file without flag O_RDWR or O_WRONLY\n");
            return -1;
        }
    }
}
...
{
    ASSERT(dir != NULL);
    return dir_read(dir);/*读取目录项的时候会紧接着调整文件偏移指针的位置*/
}
